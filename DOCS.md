...
- **Logic DMA:** Translates configuration parameters from the BAR2 register space into AXI DMA descriptors. It defines the specific source and destination addresses within the DDR3 memory for both adder and host data transfer.
- **DDR3 Memory Controller:** Provides a common command interface that connects to a memory chip to access and store data.
...
![Architecture](pci-ddr-add.drawio.svg)
