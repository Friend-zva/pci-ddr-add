module logic_adder (
    ...
    input  [255:0] m_axis_rx_tdata,
    input          m_axis_rx_tlast,
    input  [ 31:0] m_axis_rx_tuser,
    input  [ 31:0] m_axis_rx_tkeep,
    // Transmit
    input          s_axis_tx_tready,
    output         s_axis_tx_tvalid,
    output         s_axis_tx_tlast,
    output [255:0] s_axis_tx_tdata,
    output [ 31:0] s_axis_tx_tuser,
    output [ 31:0] s_axis_tx_tkeep,
  ...
  assign stream_fire = m_axis_rx_tvalid && m_axis_rx_tready;
  ...
  generate
    for (tx_dw = 0; tx_dw < 8; tx_dw = tx_dw + 1) begin : gen_tx_add16
      wire [31:0] dword;
      wire [16:0] sum16;
      assign dword = m_axis_rx_tdata[tx_dw*32+:32];
  ...
  always @(posedge clk or negedge rstn) begin
    if (!rstn) begin
      ...
      if (!stream_enable) begin
        ...
        if (stream_fire && m_axis_rx_tlast) begin
          read_desc_issued <= 1'b0;
          write_desc_issued <= 1'b0;
          busy <= 1'b0;
          done <= 1'b1;
  ...
  wire pipe_ready = s_axis_tx_tready || !pipe_tvalid;
  wire in_valid = stream_enable && read_desc_issued && write_desc_issued && m_axis_rx_tvalid;

  assign m_axis_rx_tready = stream_enable && read_desc_issued && write_desc_issued && pipe_ready;
  ...
  always @(posedge clk or negedge rstn) begin
    if (!rstn) begin
      ...
      if (pipe_ready) begin
        ...
        pipe_tlast  <= m_axis_rx_tlast;
        pipe_tkeep  <= m_axis_rx_tkeep;
        pipe_tuser  <= m_axis_rx_tuser;
      end
    end
  end

  assign s_axis_tx_tvalid  = pipe_tvalid;
  assign s_axis_tx_tdata   = pipe_tdata;
  assign s_axis_tx_tlast   = pipe_tlast;
  assign s_axis_tx_tkeep   = pipe_tkeep;
  assign s_axis_tx_tuser   = pipe_tuser;
